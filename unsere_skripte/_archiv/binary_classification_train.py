...
network_logs
#%% Spalte Scan_Type ist eine Zielgröße
# TODO: später testen, wie gut das modell performt, wenn die Spalte nicht gelöscht wird
# wir wollen zunächst ein Modell für "Intrusion" trainieren
network_logs.drop(columns=["Scan_Type"], inplace=True)
#%% One-Hot-Encoding / Dummy-Encoding
network_dummies = pd.get_dummies(network_logs, dtype=int, drop_first=True)
network_dummies
# %% unabhängige und abhängige Variablen trennen (X, y)
X = network_dummies.drop(columns=["Intrusion"])
y = network_dummies[["Intrusion"]]
...
with torch.no_grad():
    for (X_test, y_test) in test_loader:
        ...
        y_test_pred_probs = torch.sigmoid(y_test_pred_logits).numpy().flatten()
